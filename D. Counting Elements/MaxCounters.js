...
M integers, returns a sequence of integers representing the values of 
the counters.
...
function solution(N, A) {
    ...
    for (let i = 0; i < A.length; i++) {
        ...
        if (current + 1 <= N) {
            if (result[current] < currMax) result[current] = currMax
            result[current]++
    ...
    for (let j = 0; j < N; j++) {
        if (result[j] < currMax) result[j] = currMax;
    }
