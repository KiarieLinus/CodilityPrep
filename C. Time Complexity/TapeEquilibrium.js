...
function solution(A) {
    let part1Sum = arrSum(A.slice(0, 1))
    let part2Sum = arrSum(A.slice(1))
    let smallest = Math.abs(part1Sum - part2Sum)
    if (A.length === 2) return smallest

    for (let i = 1; i < A.length - 1; i++) {
        part1Sum += A[i]
        part2Sum -= A[i]
        let current = Math.abs(part1Sum - part2Sum)
